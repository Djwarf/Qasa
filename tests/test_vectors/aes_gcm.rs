...
pub fn standard_test_vectors() -> Vec<AesGcmTestVector> {
    ...
    // Test vector 1: Basic encryption with 32-byte key (AES-256)
    let key_1 = [
    ...
    let plaintext_3 = b"AES-256 test vector";
    let nonce_3 = [
        0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
        0x28, 0x29, 0x2a, 0x2b,
    ];
    ...
    // Test vector 4: 32-byte key (AES-256) with AAD
    let key_4 = [
...
pub fn negative_test_vectors() -> Vec<(Vec<u8>, Vec<u8>, Vec<u8>, Option<Vec<u8>>)> {
    ...
    let (ciphertext, _) = aes::encrypt(plaintext, &key, Some(aad))
        .expect("Failed to encrypt plaintext");
    
    // Case 1: Tampered ciphertext (flip a bit)
    let mut tampered_ciphertext = ciphertext.clone();
    if !tampered_ciphertext.is_empty() {
        let tamper_index = tampered_ciphertext.len() / 2;
        tampered_ciphertext[tamper_index] ^= 0x01;
    }
    ...
    if !tampered_aad.is_empty() {
        let tamper_index = tampered_aad.len() / 2;
        tampered_aad[tamper_index] ^= 0x01;
    }
