...
impl KyberKeyPair {
    ...
    pub fn encapsulate(&self) -> Result<(Vec<u8>, Vec<u8>), CryptoError> {
        ...
        let (ciphertext, shared_secret) = kyber.encapsulate(&pk).map_err(|e| {
            CryptoError::kyber_error(
                "Encapsulation failed",
                &e.to_string(),
    ...
    pub fn verify_key_pair(&self) -> Result<(), CryptoError> {
        ...
        let (ct, ss1) = kyber.encapsulate(&pk).map_err(|e| {
            CryptoError::kyber_error(
                "Encapsulation failed",
                &e.to_string(),
                error_codes::KYBER_ENCAPSULATION_FAILED,
        ...
        let ss2 = kyber.decapsulate(&sk, &ct).map_err(|e| {
            CryptoError::kyber_error(
                "Decapsulation failed",
                &e.to_string(),
                error_codes::KYBER_DECAPSULATION_FAILED,
    ...
    /// Test decapsulation for constant-time execution
    #[cfg(feature = "constant-time-testing")]
    pub fn decapsulate_test_constant_time(
        &self,
        ciphertext: &[u8],
        config: &crate::security::constant_time::ConstantTimeConfig,
    ) -> Result<crate::security::constant_time::ConstantTimeResult, CryptoError> {
...
impl KyberPublicKey {
    ...
    /// Test encapsulation for constant-time execution
    #[cfg(feature = "constant-time-testing")]
    pub fn encapsulate_test_constant_time(
        &self,
        config: &crate::security::constant_time::ConstantTimeConfig,
    ) -> Result<crate::security::constant_time::ConstantTimeResult, CryptoError> {
        use crate::security::constant_time::verify_constant_time;
...
mod tests {
    ...
    fn test_encapsulation_decapsulation() {
        ...
        assert_eq!(
            shared_secret1.len(),
            KyberVariant::Kyber768.shared_secret_size()
        );
