...
func TestCryptoFFI(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Failed to get crypto provider: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to generate Kyber key pair: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to generate Dilithium key pair: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to encrypt message: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to decrypt message: %v\n", err)
	}
	...
	if string(decrypted) != string(plaintext) {
		t.Fatalf("ERROR: Decrypted message does not match original message!")
	} else {
	...
	if err != nil {
		t.Fatalf("Failed to sign message: %v\n", err)
	}
	...
	if valid {
		...
		t.Fatalf("ERROR: Signature verification failed!")
	}
	...
	if err != nil {
		t.Fatalf("Failed to verify signature with tampered message: %v\n", err)
	}

	if valid {
		t.Fatalf("ERROR: Signature verification passed with tampered message!")
	} else {
	...
	if err != nil {
		t.Fatalf("Failed to create Alice's key exchange handler: %v\n", err)
	}

	bobHandler, err := encryption.NewKeyExchangeHandler(provider)
	if err != nil {
		t.Fatalf("Failed to create Bob's key exchange handler: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to create initiator message: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to process initiator message: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to process response message: %v\n", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to process finalization message: %v\n", err)
	}
