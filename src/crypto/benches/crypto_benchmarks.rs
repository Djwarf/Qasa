...
fn kyber_benchmarks(c: &mut Criterion) {
    ...
    for variant in [KyberVariant::Kyber512, KyberVariant::Kyber768, KyberVariant::Kyber1024].iter() {
        group.bench_with_input(
            ...
            variant,
            |b, &variant| {
                b.iter(|| KyberKeyPair::generate(variant))
            },
        );
    }
    ...
    for variant in [KyberVariant::Kyber512, KyberVariant::Kyber768, KyberVariant::Kyber1024].iter() {
        ...
        group.bench_with_input(
            ...
            variant,
            |b, _| {
                b.iter(|| key_pair.decapsulate(&ciphertext))
            },
        );
    }
...
fn dilithium_benchmarks(c: &mut Criterion) {
    ...
    for variant in [DilithiumVariant::Dilithium2, DilithiumVariant::Dilithium3, DilithiumVariant::Dilithium5].iter() {
        group.bench_with_input(
            ...
            variant,
            |b, &variant| {
                b.iter(|| DilithiumKeyPair::generate(variant))
            },
        );
    }
    ...
    for variant in [DilithiumVariant::Dilithium2, DilithiumVariant::Dilithium3, DilithiumVariant::Dilithium5].iter() {
        ...
        group.bench_with_input(
            ...
            variant,
            |b, _| {
                b.iter(|| pub_key.verify(message, &signature))
            },
        );
    }
...
fn dilithium_optimized_benchmarks(c: &mut Criterion) {
    ...
    for variant in [DilithiumVariant::Dilithium2, DilithiumVariant::Dilithium3, DilithiumVariant::Dilithium5].iter() {
        ...
        group.bench_with_input(
            ...
            variant,
            |b, _| {
                b.iter(|| dilithium::lean_sign(message, &key_pair.secret_key, *variant))
            },
        );
        ...
        group.bench_with_input(
            ...
            variant,
            |b, _| {
                b.iter(|| dilithium::lean_verify(message, &signature, &key_pair.public_key, *variant))
            },
        );
        
        // Benchmark batch verification (3 signatures)
        let batch = vec![
            (message as &[u8], signature.as_ref(), key_pair.public_key.as_ref(), *variant),
            (message as &[u8], signature.as_ref(), key_pair.public_key.as_ref(), *variant),
            (message as &[u8], signature.as_ref(), key_pair.public_key.as_ref(), *variant),
        ];
        ...
        group.bench_with_input(
            ...
            variant,
            |b, _| {
                b.iter(|| dilithium::lean_verify_batch(&batch))
            },
        );
    }
...
criterion_group!(benches, kyber_benchmarks, dilithium_benchmarks, dilithium_optimized_benchmarks, aes_benchmarks, utils_benchmarks);
