...
impl SideChannelTester {
    ...
    fn generate_recommendations(&self, tests: &[TimingAnalysis]) -> Vec<String> {
        ...
        let vulnerable_tests: Vec<_> = tests.iter()
            .filter(|t| t.potential_vulnerability)
            .collect();
        
        if vulnerable_tests.is_empty() {
            recommendations.push("Excellent! No timing vulnerabilities detected.".to_string());
            recommendations.push("Continue regular side-channel testing as part of security audits.".to_string());
        } else {
            recommendations.push(format!("⚠️  {} potential timing vulnerabilities detected.", vulnerable_tests.len()));
            
            for test in vulnerable_tests {
                if test.test_name.contains("Kyber") {
                    recommendations.push("Consider implementing constant-time Kyber operations".to_string());
                } else if test.test_name.contains("Dilithium") {
                    recommendations.push("Review Dilithium implementation for constant-time guarantees".to_string());
                } else if test.test_name.contains("AES") {
                    recommendations.push("Ensure AES implementation uses constant-time operations".to_string());
                } else if test.test_name.contains("Memory") {
                    recommendations.push("Review memory access patterns for information leakage".to_string());
                } else if test.test_name.contains("Cache") {
                    recommendations.push("Implement cache-resistant table lookups".to_string());
                }
            }
            
            recommendations.push("Consider using dedicated constant-time cryptographic libraries".to_string());
            ...
            recommendations.push("Use hardware security modules (HSMs) for critical operations".to_string());
        }
