...
impl CryptoError {
    ...
    pub fn user_friendly_message(&self) -> String {
        match self {
            ...
            CryptoError::AesError { operation, .. } => {
                format!("Encryption operation '{}' failed. Data security may be compromised.", operation)
            },
            ...
            CryptoError::SecurityPolicyViolation { policy, severity, .. } => {
                match severity {
                    SecuritySeverity::Critical => format!("Critical security violation: {}. Immediate action required.", policy),
                    SecuritySeverity::High => format!("High-priority security issue: {}. Please address promptly.", policy),
                    SecuritySeverity::Medium => format!("Security concern: {}. Review recommended.", policy),
                    SecuritySeverity::Low => format!("Minor security notice: {}.", policy),
                }
            },
            CryptoError::MemoryError { operation, .. } => {
                format!("Memory operation '{}' failed. System may be low on secure memory.", operation)
            },
            CryptoError::RandomGenerationError { .. } => {
                "Random number generation failed. Cryptographic operations may be insecure.".to_string()
            },
            ...
            CryptoError::ProtocolError { protocol, phase, .. } => {
            ...
            CryptoError::SideChannelViolation { test_name, .. } => {
                format!("Side-channel vulnerability detected in '{}'. Security may be compromised.", test_name)
            },
            ...
            CryptoError::InvalidParameter { parameter, expected, .. } => {
                format!("Invalid parameter '{}'. Expected '{}' format.", parameter, expected)
            },
            CryptoError::UnsupportedOperation { operation, platform, .. } => {
                format!("Operation '{}' is not supported on '{}' platform.", operation, platform)
            },
            CryptoError::ResourceExhaustion { resource, .. } => {
                format!("System resource '{}' exhausted. Performance may be degraded.", resource)
            },
    ...
    pub fn technical_details(&self) -> HashMap<String, String> {
        ...
        match self {
            CryptoError::KyberError { operation, cause, context, .. } => {
                details.insert("operation".to_string(), operation.clone());
                details.insert("cause".to_string(), cause.clone());
                details.extend(context.clone());
            },
            CryptoError::DilithiumError { operation, cause, context, .. } => {
                details.insert("operation".to_string(), operation.clone());
                details.insert("cause".to_string(), cause.clone());
                details.extend(context.clone());
            },
            CryptoError::AesError { operation, cause, context, .. } => {
                details.insert("operation".to_string(), operation.clone());
                details.insert("cause".to_string(), cause.clone());
                details.extend(context.clone());
            },
            ...
            CryptoError::SecurityPolicyViolation { policy, details: policy_details, severity, .. } => {
            ...
            CryptoError::InvalidParameter { parameter, expected, actual, .. } => {
    ...
    pub fn suggested_remediation(&self) -> Option<String> {
        match self {
            CryptoError::KyberError { error_code, .. } => {
                match *error_code {
                    error_codes::KYBER_KEY_GENERATION_FAILED => {
                        Some("Ensure sufficient entropy is available. Check system random number generator.".to_string())
                    },
                    error_codes::KYBER_INVALID_KEY_SIZE => {
                        Some("Use a supported Kyber variant (Kyber512, Kyber768, or Kyber1024).".to_string())
                    },
                    _ => Some("Check Kyber algorithm parameters and input data validity.".to_string()),
                }
            },
            CryptoError::DilithiumError { error_code, .. } => {
                match *error_code {
                    error_codes::DILITHIUM_VERIFICATION_FAILED => {
                        Some("Verify the signature and public key are correct. Check for data tampering.".to_string())
                    },
                    error_codes::DILITHIUM_INVALID_KEY_SIZE => {
                        Some("Use a supported Dilithium variant (Dilithium2, Dilithium3, or Dilithium5).".to_string())
                    },
                    _ => Some("Check Dilithium algorithm parameters and input data validity.".to_string()),
                }
            },
            CryptoError::SecurityPolicyViolation { severity, .. } => {
                match severity {
                    SecuritySeverity::Critical => {
                        Some("Immediately stop current operation and review security configuration.".to_string())
                    },
            ...
            CryptoError::RandomGenerationError { .. } => {
                Some("Check system entropy sources. Consider using hardware RNG if available.".to_string())
            },
            CryptoError::MemoryError { .. } => {
                Some("Check available system memory. Consider reducing concurrent operations.".to_string())
            },
            CryptoError::HardwareAccelerationError { .. } => {
                Some("Hardware acceleration failed. Software fallback will be used automatically.".to_string())
            },
            CryptoError::UnsupportedOperation { .. } => {
                Some("Use platform-specific alternatives or update to a supported platform.".to_string())
            },
            _ => None,
        }
    }
...
mod tests {
    ...
    fn test_user_friendly_message() {
        let error = CryptoError::security_violation("constant_time", "Timing leak detected", SecuritySeverity::Critical);
