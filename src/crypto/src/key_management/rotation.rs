...
impl KeyRotationMetadata {
    ...
    pub fn update_after_rotation(&mut self, old_key_id: String) {
        ...
        if !self.policy.keep_old_keys {
            ...
            self.previous_key_ids
                .truncate(self.policy.old_keys_to_keep as usize);
        }
    }
    ...
    pub fn days_until_rotation(&self) -> u32 {
        let reference_time = self.last_rotated.unwrap_or(self.created_at);
        let next_rotation =
            reference_time + Duration::days(self.policy.rotation_interval_days as i64);
        let now = Utc::now();
...
pub fn save_metadata(
    ...
    let data = bincode::serialize(metadata).map_err(|e| {
        CryptoError::SerializationError(format!("Failed to serialize metadata: {}", e))
    })?;

    let mut file = File::create(&path).map_err(|e| {
        CryptoError::key_management_error(
            "Failed to create metadata file",
            &format!("Metadata file creation error: {}", e),
            "unknown",
        )
    })?;
...
pub fn load_metadata(key_id: &str, key_type: &str) -> Result<KeyRotationMetadata, CryptoError> {
    ...
    let mut file = File::open(&path).map_err(|e| {
        CryptoError::key_management_error(
            ...
            &format!("Metadata file open error: {}", e),
            "unknown",
        )
    })?;
    ...
    file.read_to_end(&mut data).map_err(|e| {
        CryptoError::key_management_error(
            ...
            &format!("Metadata read error: {}", e),
            "unknown",
        )
    })?;

    let metadata = bincode::deserialize(&data).map_err(|e| {
        CryptoError::SerializationError(format!("Failed to deserialize metadata: {}", e))
    })?;
    ...
    for entry in entries {
        let entry = entry.map_err(|e| {
            CryptoError::key_management_error(
                ...
                &format!("Directory entry read error: {}", e),
                "unknown",
            )
        })?;
        let path = entry.path();
    ...
    for entry in entries {
        ...
        match get_key_age(key_id, key_type) {
            Ok(age_summary) => {
                ...
                let days_since_rotation = age_summary
                    .days_since_rotation
...
impl KeyAgeSummary {
    ...
    pub fn status_description(&self) -> String {
        if self.rotation_recommended {
            ...
            format!(
                "Key rotation will be needed in {} days",
                self.days_until_next_rotation
            )
        } else {
            format!(
                "Key is valid for {} more days",
                self.days_until_next_rotation
            )
        }
    }
    ...
    for entry in fs::read_dir(key_dir)? {
        ...
        if entry.file_type()?.is_dir() {
            ...
            if metadata_path.exists() {
                ...
                let days_since_rotation = metadata
                    .last_rotated
                    .map(|last_rotated| (current_time - last_rotated).num_days() as u32);

                // Calculate days until next rotation
                let reference_time = metadata.last_rotated.unwrap_or(metadata.created_at);
                let next_rotation =
                    reference_time + Duration::days(metadata.policy.rotation_interval_days as i64);
...
mod tests {
    ...
    fn test_metadata_save_load() {
        ...
        assert_eq!(
            loaded_metadata.policy.rotation_interval_days,
            original_metadata.policy.rotation_interval_days
        );
        assert_eq!(
            loaded_metadata.policy.keep_old_keys,
            original_metadata.policy.keep_old_keys
        );
        assert_eq!(
            loaded_metadata.previous_key_ids,
            original_metadata.previous_key_ids
        );
    }
    ...
    fn test_new_key_age_methods() {
        ...
        assert!(
            age >= 49 && age <= 51,
            "Key age should be approximately 50 days, got {}",
            age
        );
        ...
        assert!(
            days_since >= 19 && days_since <= 21,
            "Days since rotation should be approximately 20, got {}",
            days_since
        );
        ...
        assert!(
            days_until >= 69 && days_until <= 71,
            "Days until rotation should be approximately 70, got {}",
            days_until
        );
    }
    ...
    fn test_get_key_age() {
        ...
        let mock_key_dir = || -> PathBuf { temp_path.to_path_buf() };
