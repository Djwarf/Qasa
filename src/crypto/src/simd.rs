    ...
    impl SimdPolynomial {
        ...
        pub fn multiply(&self, other: &Self) -> CryptoResult<Self> {
            match self.algorithm {
                ...
                SimdAlgorithm::AVX2 => {
                    let poly1 =
                        x86_simd::X86SimdPolynomial::from_coefficients(self.coefficients.clone());
                    let poly2 =
                        x86_simd::X86SimdPolynomial::from_coefficients(other.coefficients.clone());
                ...
                #[cfg(not(target_arch = "x86_64"))]
                SimdAlgorithm::AVX2 => Err(CryptoError::invalid_parameter(
                    "AVX operations",
                    "x86_64",
                    std::env::consts::ARCH,
                )),
...
pub fn benchmark_simd_performance(
    input_size: usize,
    iterations: usize,
) -> CryptoResult<SimdBenchmarkResult> {
    ...
    let scalar_poly1 =
        SimdPolynomial::with_algorithm(poly1.coefficients().to_vec(), SimdAlgorithm::Scalar)?;
    let scalar_poly2 =
        SimdPolynomial::with_algorithm(poly2.coefficients().to_vec(), SimdAlgorithm::Scalar)?;
