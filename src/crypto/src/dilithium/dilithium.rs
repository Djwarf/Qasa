...
impl DilithiumVariant {
    ...
    pub fn for_constrained_environment(min_security_level: u8, available_memory_kb: usize) -> Option<Self> {
...
impl LeanDilithium {
    ...
    fn get_sig(&mut self) -> Result<&mut Sig, CryptoError> {
        if self.sig_instance.is_none() {
            ...
            self.sig_instance = Some(Sig::new(alg).map_err(|e| CryptoError::OqsError(e.to_string()))?);
        }
    ...
    pub fn verify(&mut self, message: &[u8], signature: &[u8], public_key: &[u8]) -> Result<bool, CryptoError> {
        let sig = self.get_sig()?;
    ...
    pub fn verify_batch(&mut self, batch: &[(&[u8], &[u8], &[u8])]) -> Result<Vec<bool>, CryptoError> {
        if batch.is_empty() {
            return Ok(Vec::new());
        }
...
///
/// The signature or an error
pub fn lean_sign(message: &[u8], secret_key: &[u8], variant: DilithiumVariant) -> Result<Vec<u8>, CryptoError> {
...
///
/// `true` if the signature is valid, `false` otherwise
pub fn lean_verify(message: &[u8], signature: &[u8], public_key: &[u8], variant: DilithiumVariant) -> Result<bool, CryptoError> {
    let mut lean = LeanDilithium::new(variant);
...
pub fn lean_verify_batch(batch: &[(&[u8], &[u8], &[u8], DilithiumVariant)]) -> Result<Vec<bool>, CryptoError> {
    if batch.is_empty() {
        return Ok(Vec::new());
    }
...
mod tests {
    ...
    fn test_dilithium_key_generation() {
        ...
        // Verify key sizes
        assert_eq!(key_pair.public_key.len(), DilithiumVariant::Dilithium3.public_key_size());
        assert_eq!(key_pair.secret_key.len(), DilithiumVariant::Dilithium3.secret_key_size());
    }
    ...
    fn test_all_variants() {
        ...
        for variant in [DilithiumVariant::Dilithium2, DilithiumVariant::Dilithium3, DilithiumVariant::Dilithium5].iter() {
    ...
    fn test_lean_dilithium() {
        ...
        let is_valid = lean.verify(message, &signature, &key_pair.public_key).unwrap();
        assert!(is_valid);
        ...
        let is_valid2 = lean.verify(message, &signature2, &key_pair.public_key).unwrap();
        assert!(is_valid2);
    }
    ...
    fn test_streamlined_functions() {
        ...
        let is_valid = lean_verify(message, &signature, &key_pair.public_key, key_pair.algorithm).unwrap();
    ...
    fn test_batch_verification() {
        ...
        let batch = vec![
            (message1 as &[u8], signature1.as_ref(), key_pair1.public_key.as_ref()),
            // This should fail (wrong key)
            (message2 as &[u8], signature2.as_ref(), key_pair1.public_key.as_ref()),
        ];
        ...
        let batch_mixed = vec![
            (message1 as &[u8], signature1.as_ref(), key_pair1.public_key.as_ref(), DilithiumVariant::Dilithium2),
            (message2 as &[u8], signature2.as_ref(), key_pair2.public_key.as_ref(), DilithiumVariant::Dilithium3),
            // This should fail (wrong variant)
            (message1 as &[u8], signature1.as_ref(), key_pair1.public_key.as_ref(), DilithiumVariant::Dilithium3),
        ];
