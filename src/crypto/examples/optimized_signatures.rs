use qasa_crypto::{
    dilithium::{self, DilithiumKeyPair, DilithiumVariant, LeanDilithium},
    error::CryptoError,
};
...
fn main() -> Result<(), CryptoError> {
    ...
    let variant =
        DilithiumVariant::for_constrained_environment(min_security_level, available_memory_kb)
            .expect("No suitable variant for these constraints");

    println!(
        "Selected variant: {} (requires {}KB, security level {})",
        variant,
        variant.memory_requirement_kb(),
        variant.security_level()
    );

    // Message to sign
    let message =
        b"This is a message signed with a quantum-resistant algorithm on a constrained device";
    ...
    println!("\n[2] Using streamlined functions for one-off operations");
    {
        ...
        println!(
            "  - Verified signature using lean_verify: {}",
            if is_valid { "valid" } else { "invalid" }
        );
    }
